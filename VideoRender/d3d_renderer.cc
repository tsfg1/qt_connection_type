 ...
 *
 *  Use of this source code is governed by a BSD-style license  that can be found in the LICENSE file in the root of the source  tree. An additional intellectual property rights grant can be found
 *  in the file PATENTS.  All contributing project authors may  be found in the AUTHORS file in the root of the source tree.
 */
...
#define D3DFVF_CUSTOMVERTEX (D3DFVF_XYZ|D3DFVF_TEX1)
	...
	D3dRenderer::D3dRenderer(const void* hwnd, size_t width, size_t height)
		: 
		hwnd_((HWND)hwnd),
		width_(width),
		height_(height),
	...
	void D3dRenderer::OnFrame(const webrtc::VideoFrame& frame) {
		...
		if (static_cast<size_t>(videoFrame->width()) != width_ || static_cast<size_t>(videoFrame->height()) != height_)
		{
			if (static_cast<size_t>(videoFrame->width()) == 0 || static_cast<size_t>(videoFrame->height()) == 0)
			{
				return;
			}
		...
		ConvertFromI420(video_frame, VideoType::kARGB, 0, static_cast<uint8_t*>(lock_rect.pBits));
