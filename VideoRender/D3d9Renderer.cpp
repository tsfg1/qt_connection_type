...
namespace webrtc {
	...
	D3d9Renderer::D3d9Renderer(const void *hwnd, bool fStart):
	...
	void D3d9Renderer::OnFrame(const webrtc::VideoFrame & frame)
	{	
		...
		if (!m_fRuning)
		{
			...
			this->Invoke<void>(RTC_FROM_HERE, rtc::Bind(&D3d9Renderer::renderFrame, this, &frame));
		}
#else
			renderFrame(&frame); //非线程渲染模式
#endif	
	}
	void D3d9Renderer::resetRenderDevice(void * winId)
	{
	...
	void D3d9Renderer::setRenderAspect(float aspect)
	{
	...
	void D3d9Renderer::saveFrame(const webrtc::VideoFrame& frame, const std::string filename)
	{
	...
	uint8_t* D3d9Renderer::Data(const uint8_t* y)
	{
		return const_cast<uint8_t*>(y);
	}

	void D3d9Renderer::renderFrame(const webrtc::VideoFrame * frame)
	{
		RECT rcClient;
		...
		libyuv::I420Scale(buffer->GetI420()->DataY(), buffer->GetI420()->StrideY(),
